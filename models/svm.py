...
def train_svm(X_train, y_train, X_val, y_val, X_test, y_test, save_data):
    model = SVC(random_state=42)
    model.fit(X_train, y_train)
    #joblib.dump(model, save_data + "svm.joblib")
        # Evaluate on the validation set
    val_acc, val_pre, val_rec, val_f1 = evaluate_model(model, X_val, y_val)
    print("SVM Validation - Accuracy:", val_acc, "Precision:", val_pre, "Recall:", val_rec, "F1 Score:", val_f1)
    ...
    print("SVM Test - Accuracy:", test_acc, "Precision:", test_pre, "Recall:", test_rec, "F1 Score:", test_f1)
